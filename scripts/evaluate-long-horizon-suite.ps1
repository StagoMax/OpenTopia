  ...
  [ordered]@{
    ...
    runs = @($taskRuns | ForEach-Object {
      [ordered]@{
        ...
        blockedEquivalentToolCalls = $_.result.trajectoryMetrics.blockedEquivalentToolCalls
        blockedCompletionModeToolCalls = $_.result.trajectoryMetrics.blockedCompletionModeToolCalls
        blockedImplementationModeToolCalls = $_.result.trajectoryMetrics.blockedImplementationModeToolCalls
