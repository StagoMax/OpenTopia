...
flowchart TB
    ...
    Sandbox["Execution Sandbox<br/>local policy / OS sandbox / Docker"]
...
1. **Local constrained**：默认本地执行，有工作区边界、命令策略、网络策略、输出限制。
2. **OS sandbox**：macOS Seatbelt、Windows 限权/Job Object、Linux namespace/seccomp/bubblewrap。
3. **Docker sandbox**：远程/不可信任务、团队版、浏览器/服务端隔离。

推荐参考：

- Codex exec/sandboxing：本地执行、安全策略、输出截断、取消、超时。
- OpenHands Docker sandbox：容器生命周期、端口映射、agent server、pause/resume。
...
OpenHands 的容器化设计适合第二阶段引入：把每个 workspace/session 放到一个 sandbox，并暴露 agent server、VS Code、端口转发。但第一阶段可以先实现 Codex 式本地执行策略。
...
| Docker sandbox | OpenHands | 第二阶段参考/适配 |
...
- 第一版可只有 local constrained 实现。
- Docker/remote 后续作为实现替换。
...
11. 加 Docker sandbox。
12. 加 GitHub/Linear/Docs/Browser 等 Work tools。
