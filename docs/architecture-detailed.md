...
桌面端还提供正式的自然语言 Skill 创建流程。`POST /api/skills/generate` 使用当前配置的真实 Provider 和严格 JSON Schema 生成结构化草稿，但不写入磁盘；用户可以检查和编辑名称、触发描述、指令、界面元数据及资源文件。确认后，`POST /api/skills` 会再次执行全部校验，再创建 Skill。

项目 Skill 固定写入已注册工作区的 `.agents/skills/<name>/`，用户 Skill 写入 `$CODEX_HOME/skills/<name>/`，未设置 `CODEX_HOME` 时回退到 `~/.codex/skills/<name>/`。客户端不能提交绝对目标路径。名称只允许小写 ASCII、数字和单连字符；资源只能位于 `references/`、`scripts/` 或 `assets/`，并受文件数、单文件和总字节上限约束。服务端在目标根目录创建随机临时目录，写入并同步所有文件后执行同卷原子重命名；已有同名目标返回 `409 Conflict`，不会覆盖或合并。
