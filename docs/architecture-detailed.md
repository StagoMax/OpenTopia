...
Agent Turn 不使用固定工具轮数或总运行时长来判断任务是否完成。每当模型返回一个或多个工具调用，运行时执行工具、追加结构化结果并再次请求模型；模型可以根据新观察修改计划、增加步骤、换一种方法或继续验证。Provider 正常终止、返回非空助手文本且不含工具调用时，只产生一个 final candidate，不会直接把 Turn 标记为成功。

Provider 响应先归一化为 `ModelDecision::Act`、`Final` 或 `Incomplete`。Chat Completions 的 `finish_reason` 和 Responses API 的 completed/incomplete 终止事件都会保留；`length`、内容过滤、空白响应、没有终止事件的流中断都进入 incomplete/失败分支，不能成为最终答复。Final candidate 还要通过 `FinalizationGuard`：没有待执行工具或审批、没有活动或未收集的后代 Agent、没有 `in_progress` 计划步骤，并且结构化计划或 `complete_task` 声明需要验证时已有 evidence。门禁未通过会把结构化阻塞原因返回模型，允许继续执行或修改计划；连续忽略门禁则失败，不伪造成功。门禁通过后，运行时仍会根据结构化终态分类：存在 blocked 计划步骤时产生 `Blocked`，`complete_task.remainingWork` 非空或未声明当前范围已完成的 deferred/cancelled 步骤产生 `Partial`，只有完整完成才产生 `Completed` 并把 Turn 标记为 `succeeded`。
...
长时间没有进展的任务由分段 rollout 审查约束。运行时只统计主 Agent 的模型调用：完成第 90、180 和 270 轮后，如果模型仍请求继续使用工具，会复用 guardian reviewer 检查实际进展。Reviewer 只有在存在具体、可行且能产生可验证进展的下一步时才允许进入下一段；不同工具或不同步骤但没有实质进展同样应停止。第 270 轮是硬上限，不会启动第 271 轮。Reviewer 调用和工具调用不计入该轮次。

成本失控还可以由可选的 rollout token budget 提前约束。预算按 Provider 报告的输出 token 和未缓存输入 token 加权累计；达到 25% 和 10% 剩余额度时向模型注入提醒，任一次模型响应记账后耗尽额度都会终止 Turn。预算默认关闭，可在 Provider 设置中按模型价格和任务规模启用。单个工具仍有自己的超时，用户也可以随时取消 Turn。

上下文窗口是另一条独立边界。工具历史接近窗口阈值时，运行时把较早的已完成调用压缩为摘要，并保留原始目标、持久计划、近期工具结果和验证信息。这样动态增加步骤不依赖模型永久记住所有原文，而依赖“当前目标 + 可恢复计划 + 压缩后的已知事实”持续回注。

**为什么不使用固定轮数？** 轮数无法区分长程任务和死循环；一次搜索可能解决问题，复杂迁移也可能合理地运行几十轮。类型化模型决策提供协议终点，FinalizationGuard 校验运行时就绪状态，加权预算负责资源上限，计划和摘要负责目标连续性，各自职责互不混淆。
...
Skill 创建复用普通 Agent 工具调用链，不设专用生成接口或预览弹窗。内置 `create_skill` 工具向模型声明名称、触发描述、指令、作用域、界面元数据和可选资源的结构；当用户要求“总结并创建 Skill”时，模型直接根据当前对话归纳可复用流程并调用该工具。工具结果会回到同一个模型回合，模型最终只需向用户说明创建的 Skill 名称、用途、路径和文件，用户可在创建后自行检查。

工具默认创建用户 Skill；只有用户明确要求当前项目专用时才使用项目作用域。项目 Skill 固定写入已注册工作区的 `.agents/skills/<name>/`，用户 Skill 写入 `$CODEX_HOME/skills/<name>/`，未设置 `CODEX_HOME` 时回退到 `~/.codex/skills/<name>/`。模型不能提交绝对目标路径。名称只允许小写 ASCII、数字和单连字符；资源只能位于 `references/`、`scripts/` 或 `assets/`，并受文件数、单文件和总字节上限约束。执行仍经过 Policy Engine 和审批边界；实现会在目标根目录创建随机临时目录，写入并同步所有文件后执行同卷原子重命名，已有同名目标返回冲突且不会覆盖或合并。
