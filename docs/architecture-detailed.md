...
Agent Turn 不设置总工具轮次或总运行时长上限。只要工具调用、计划状态或验证证据仍在产生新的任务状态，执行就可以持续一小时以上，不会因为经过固定轮数或时间而要求用户批准继续。

循环防护只针对“回到同一个问题状态”：在没有有意义进展的区间内，规范化后完全相同的工具调用最多重复 3 次；由多个不同调用组成的重复序列连续出现 3 次时也会被识别。运行时先把结构化纠偏结果返回给模型，要求改变方法、产生新状态或如实结束；模型连续忽略 3 次纠偏后，才以未完成状态停止该重复模式。不同文件、不同命令和不同证据不会因为数量多而触发限制。

上下文窗口是模型的物理约束，不是用户审批边界。工具历史接近窗口阈值时，运行时自动把较早的已完成调用压缩成持久摘要，同时保留最近调用、计划和验证状态；不会生成“继续执行”审批。通过验证命令后，系统仍会进入完成模式，仅保留 `update_plan` 和 `complete_task` 等收尾动作。

**为什么不采用硬预算？** 固定轮数和固定时长无法区分长程任务与死循环，容易把正常进展切碎并制造无意义审批。基于规范化调用、重复序列和进展重置的判定直接约束无效重复，而把成本控制、工具自身超时、用户取消和评测 Runner 截止时间留在各自独立的边界处理。
...
1. Agent 写入待决 `Approval`，持久化当前 Provider 对话、已完成工具结果、待执行调用、自动压缩摘要和循环防护状态。
