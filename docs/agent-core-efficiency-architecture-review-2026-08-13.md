...
失败恢复也暴露了“按参数判断重复、却不理解共同失败原因”的局限：

- `78d58398` 有 5 次失败工具调用，包括两次相同的 Shell 沙箱准备根因、一次超过表格范围限制、一次把 Tool Artifact ID（工具产物编号）误当作用户附件、一次读取并不存在的工作区模板路径。
- `4b1472a9` 有 3 次 Shell 调用，都因同一个 `PrepareSandbox` 根因失败；命令参数不同，所以精确调用签名检测不会把它们视为同一失败循环。

因此 Error Recovery（错误恢复）不能只看“是否重复了完全相同的调用”，还要对 Failure Cause（失败原因）、Capability Assumption（能力假设）和 Resource Identity（资源身份）做归一化。一个不可用的执行路径在根因未变化时不应靠换命令继续试探。
...
本文不根据外部表现反推 Codex 的专有内部实现。可核实的 [OpenAI Model Guidance](https://developers.openai.com/api/docs/guides/latest-model) 只作为设计原则参考：系统提示应保持精简并避免重复、只暴露任务相关工具、可预测且有界的工具密集阶段适合在程序化执行中聚合和压缩结果，而且只有在现有质量评测继续通过时，较少的调用、轮次、Token 或延迟才算改进。这些原则与本文结论一致，但不是对 Codex 内部架构的证明。
