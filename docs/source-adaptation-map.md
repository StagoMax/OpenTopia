...
| Codex | `J:\Project\codex cli\codex\codex-rs\core\src\tools\handlers\multi_agents_v2\`、`agent/control.rs`、`agent/registry.rs` | Agent Thread、规范任务路径、spawn/message/followup/interrupt/list/wait、历史派生、Profile 与 mailbox | `subagents.rs`、`agent_profiles.rs`、`tools.rs` 的 Agent registry、直接消息、连续回合、权限继承和事件；按 OpenTopia 架构重写 |
...
| Tools | `crates/opentopia-core/src/tools.rs` | Codex apply_patch/shell/read/write/search/plan/multi-agent tool surface | list/read/write/search/shell/diff/apply_patch/update_plan/spreadsheet/browser，以及 spawn/message/followup/interrupt/list_agents/wait；全部进入现有工具循环 |
...
| Agent collaboration runtime | `subagents.rs`、`agent_profiles.rs`、server `ServerSubagentExecutor` | Codex V2 Agent Control + Goose subagent execution | `/root/...` 稳定身份、树内直连 mailbox、连续 follow-up 回合、父历史派生、Profile、并发/深度限制、真实 AgentCore、SQLite/SSE 与递归取消 |
