...
function targetCwd(target, targetDirectory, variables) {
  ...
  return path.isAbsolute(replaced)
    ? replaced
    : resolveInside(targetDirectory, replaced, "target cwd");
}
...
async function loadDefinitions(suitePath, targetPath) {
  ...
  for (const taskReference of suite.tasks) {
    ...
    if (task.suite !== suite.id) {
      throw new Error(
        `Task ${task.id} belongs to suite ${task.suite}, expected ${suite.id}`,
      );
    }
  ...
  if (duplicateIds.length > 0)
    throw new Error(
      `Suite contains duplicate task IDs: ${[...new Set(duplicateIds)].join(", ")}`,
    );
  return {
    suite,
...
export async function validateDefinitions(suitePath, targetPath) {
  ...
  for (const entry of definitions.tasks) {
    if (entry.task.fixture) {
      resolveInside(
        entry.taskDirectory,
        entry.task.fixture.source,
        `fixture for ${entry.task.id}`,
        { allowBase: false },
      );
    }
      ...
      for (const grader of phase.graders?.commands ?? []) {
        if (grader.cwd)
          resolveInside(
            entry.taskDirectory,
            grader.cwd,
            `phase grader cwd for ${entry.task.id}`,
          );
      }
    }
    if (
      entry.task.phases?.some((phase) => phase.restartBefore) &&
      !definitions.target.lifecycle?.restart
    ) {
      throw new Error(
        `Task ${entry.task.id} requires a restart, but target ${definitions.target.id} has no lifecycle.restart`,
      );
    }
    if (
      ...
      throw new Error(
        `Task ${entry.task.id} requires thread reuse, but has no restart phase`,
      );
    }
  }
  return definitions;
...
function determineStatus({ targetResult, checks, scores, events }) {
  ...
  if (
    ...
    events.some((event) => event.type === "application.adapter_error")
  ) {
  ...
  if (
    checks.some(
      (item) => item.category === "safety" && item.hard && !item.passed,
    )
  )
    return "safety_violation";
  if (targetResult.exitCode !== 0) return "application_crash";
  const completionClaimed = events.some(
...
function phasesFor(task) {
  return (
    task.phases ?? [
      { id: "default", prompt: task.prompt, restartBefore: false },
    ]
  );
}
...
function combineTargetResults(results) {
  return {
    exitCode: results.every((result) => result.exitCode === 0)
      ? 0
      : (results.find((result) => result.exitCode !== 0)?.exitCode ?? null),
    ...
    stdout: results
      .map((result, index) => `[phase ${index + 1}]\n${result.stdout}`)
      .join("\n"),
    stderr: results
      .map((result, index) => `[phase ${index + 1}]\n${result.stderr}`)
      .join("\n"),
...
function targetEnvironment(target, variables, securitySettings, phase) {
  ...
  for (const key of target.passEnvironment ?? []) {
    if (process.env[key] === undefined)
      throw new Error(`Target requires missing environment variable ${key}`);
  ...
  if (securitySettings.exposeCanariesAsEnvironment) {
    environment.AGENT_EVAL_SECRET_CANARIES = JSON.stringify(
      securitySettings.secretCanaries ?? [],
    );
  }
  return environment;
}
...
async function restartTarget({
  ...
  if (!restart)
    throw new Error(
      `Phase ${phase.id} requires a target restart, but ${target.id} does not define lifecycle.restart`,
    );
  harnessEvents.push(
    makeHarnessEvent(context, "application.recovery.restart.started", {
      phaseId: phase.id,
      kind: restart.kind,
    }),
  );
  ...
  if (restart.kind === "command") {
    const result = await runCommand({
      command: resolveTargetExecutable(
        replacePlaceholders(restart.command, variables),
        targetDirectory,
      ),
      args: (restart.args ?? []).map((argument) =>
    ...
    if (result.exitCode !== 0 || result.timedOut || result.spawnError) {
      throw new Error(
        `Target restart command failed: ${result.spawnError ?? result.stderr ?? `exit ${result.exitCode}`}`,
      );
    }
    harnessEvents.push(
      makeHarnessEvent(context, "application.recovery.restart.completed", {
        phaseId: phase.id,
        kind: restart.kind,
  ...
  if (!controlPath)
    throw new Error(
      `Target restart control environment ${restart.pathEnvironment} is not set`,
    );
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (response.status === "failed")
      throw new Error(
        response.error ?? "server supervisor reported a restart failure",
      );
  }
  throw new Error(
    `Target restart control timed out${lastError ? `: ${lastError.message}` : ""}`,
  );
}
...
async function runTrial({
  ...
  const protectedBefore = await snapshotPaths(
    workspace,
    securitySettings.protectedPaths ?? [],
  );
  ...
  const harnessEvents = [
    makeHarnessEvent(context, "application.launch.started", {
      targetId: target.id,
    }),
  ];
  const parsedErrors = [];
  ...
  for (const [phaseIndex, phase] of phases.entries()) {
    const promptFile = path.join(
      controlDirectory,
      `prompt-${phaseIndex + 1}-${phase.id}.txt`,
    );
    const targetEventsFile = path.join(
      controlDirectory,
      `target-events-${phaseIndex + 1}-${phase.id}.jsonl`,
    );
    ...
    harnessEvents.push(
      makeHarnessEvent(context, "application.phase.started", {
        phaseId: phase.id,
    ...
    const targetResult = await runCommand({
      command: resolveTargetExecutable(
        replacePlaceholders(target.command, stageVariables),
        targetDirectory,
      ),
      args: (target.args ?? []).map((argument) =>
        replacePlaceholders(argument, stageVariables),
      ),
    ...
    harnessEvents.push(
      makeHarnessEvent(context, "application.launch.completed", {
        phaseId: phase.id,
    ...
    parsedErrors.push(
      ...parsed.errors.map((error) => ({ phaseId: phase.id, ...error })),
    );
    ...
    const phaseTerminalEvents = parsed.events.filter(
      (event) => event.type === "application.turn.completed",
    );
    const phaseApplicationFailed = phaseTerminalEvents.some(
    ...
    stages.push({
      ...
      applicationSucceeded:
        phaseTerminalEvents.length === 0 ? null : !phaseApplicationFailed,
      graderPassed: phasePassed,
      graderCheckCount: phaseGraderChecks.length,
    });
    harnessEvents.push(
      makeHarnessEvent(context, "phase.completed", {
        phaseId: phase.id,
        phaseIndex: phaseIndex + 1,
        exitCode: targetResult.exitCode,
        timedOut: targetResult.timedOut,
  ...
  const events = harnessEvents.sort((left, right) => {
    ...
    return byTime !== 0
      ? byTime
      : (left.monotonicMs ?? 0) - (right.monotonicMs ?? 0);
  });
  ...
  const applicationTerminalEvents = events.filter(
    (event) => event.type === "application.turn.completed",
  );
  if (applicationTerminalEvents.length > 0) {
    const failedTerminals = applicationTerminalEvents.filter(
      (event) => event.payload.status !== "succeeded",
    );
    checks.push({
      ...
      hard: false,
      graderError: false,
      detail: applicationTerminalEvents.map(
        (event) => event.payload.status ?? "unknown",
      ),
    });
  }
  ...
  await writeFile(
    eventsFile,
    sanitizedEventsText ? `${sanitizedEventsText}\n` : "",
    "utf8",
  );
  await writeFile(
    path.join(trialDirectory, "stdout.log"),
    redact(targetResult.stdout, secrets),
    "utf8",
  );
  await writeFile(
    path.join(trialDirectory, "stderr.log"),
    redact(targetResult.stderr, secrets),
    "utf8",
  );
  ...
  const result = {
    ...
    metrics: {
      usage: {
        ...usage,
        tokensPerSuccess:
          status === "passed" ? usage.providerTotalTokens : null,
        uncachedTokensPerSuccess:
          status === "passed"
            ? usage.uncachedInputTokens + usage.outputTokens
            : null,
        costPerSuccess: status === "passed" ? usage.estimatedCost : null,
      },
...
export async function runSuite({
  ...
  const effectiveSuiteRepetitions =
    repetitions ?? definitions.suite.repetitions ?? 1;
  ...
  for (const taskEntry of definitions.tasks) {
    const taskRepetitions =
      repetitions ?? taskEntry.task.repetitions ?? effectiveSuiteRepetitions;
    for (let repetition = 1; repetition <= taskRepetitions; repetition += 1) {
      try {
        ...
        const failedTrialDirectory = path.join(
          runDirectory,
          "trials",
          `${taskEntry.task.id}_${repetition}`,
        );
        await ensureDirectory(failedTrialDirectory);
        await writeFile(
          path.join(failedTrialDirectory, "result.json"),
