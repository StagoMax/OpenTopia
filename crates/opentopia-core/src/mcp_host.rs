...
impl McpExtensionHost {
    ...
    pub async fn restart_server(
        ...
        let client = match McpStdioClient::spawn(config.clone()).await {
            Ok(client) => client,
            Err(err) => {
...
impl McpStdioClient {
        ...
        E: AsyncRead + Unpin + Send + 'static,
    {
        ...
        let reader_task =
            spawn_stdout_reader(stdout, pending.clone(), server_id, server_name.clone());
            ...
            match lines.next_line().await {
                Ok(Some(line)) => {
                    ...
                    match parse_json_rpc_line(&line) {
                        ...
                        Ok(McpIncomingMessage::Request { method, .. }) => {
                            warn!(%server_id, %server_name, %method, "MCP server sent a client request; client request handling is not implemented yet");
                        }
                        Err(err) => {
                            let message = err.to_string();
