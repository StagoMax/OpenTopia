...
The runtime may expose `tool_search` instead of sending every optional tool schema up front. Use it only when the visible tools do not cover a needed capability. Search by the action you need, then use a returned tool after its schema appears on the next round; do not guess names or arguments for tools that have not been revealed.
