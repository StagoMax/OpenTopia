...
> 状态：Implementation in progress（实施中）
>
> 日期：2026-08-17
>
...
尚未完成的切换：Server 的统一 Agent Run 入口与 Snapshot Deriver/Activity Source 生产适配器接线、旧 `SubagentScheduler`/旧表/旧事件删除，以及桌面端新投影。当前代码已经完成领域、持久化和工具协议边界，但尚未完成生产运行路径切换。
