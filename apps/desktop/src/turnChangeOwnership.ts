...
export function turnHasSuccessfulWorkspaceWrite(
  ...
  return events.some(
    (event) =>
      event.turnId === turnId && isSuccessfulWorkspaceWriteEvent(event),
  );
}

export function isSuccessfulWorkspaceWriteEvent(event: AgentEvent): boolean {
  if (event.payload.type !== "tool_call_finished") return false;
...
function turnHasTerminalEvent(events: AgentEvent[], turnId: string): boolean {
  return events.some(
    (event) =>
      event.turnId === turnId && terminalTurnEventTypes.has(event.payload.type),
  );
}
