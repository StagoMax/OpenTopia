    ...
    <div className="enterprise-page enterprise-trust">
      ...
      >
        <p className="enterprise-page__lede">
          聚合 Connection 健康、运行失败、HumanTask 和未发布版本。执行权限仍由 DeploymentSnapshot 与调用时 live gate 决定，本页不替代授权边界。
        </p>
      ...
      <Panel title="Execution invariants / 执行不变量">
        <ul className="enterprise-invariants">
          ...
          <li><ShieldCheck aria-hidden="true" size={16} /><span><strong>Least privilege / 最小权限</strong><small>节点权限只能从模板、工作流和部署逐层收窄，不能从 Thread MCP 状态扩权。</small></span></li>
