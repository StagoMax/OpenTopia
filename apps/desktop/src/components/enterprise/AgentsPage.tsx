    ...
    <div className="enterprise-page">
      ...
      >
        <p className="enterprise-page__lede">
          Agent 是从已发布模板创建的稳定执行身份；它的模板版本、Connection 操作和资源范围在创建时冻结。
        </p>
        <ol className="enterprise-card-list">
          {snapshot.agents.map((agent) => (
            <li key={agent.id}>
              ...
              <span>
                <strong>{agent.templateId}@{agent.templateVersion}</strong>
                <small>Agent {shortId(agent.id)} · Thread {shortId(agent.threadId)}</small>
              </span>
